...
<div style="font-family: Montserrat, times, sans-serif;">
<div id="app-block" style="display:none">
  ...
  <div class="col1"><div class="col1-inner">
    ...
    <div style="background-color:#333; color:#333; padding:5px;">
      <table style="width:100%">
       ...
       <table style="width:100%">
        <tr>       
           <td style="width:30%">
              <div class="solar-title">SOLAR NOW</div>
              <div class="solarpower-value"><span id="solar_now">0</span></div>
          </td>
          <td style="text-align:right">
              <div class="solar-title">SOLAR TODAY</div>
              <div class="solarpower-value"><span id="solar_kwh_today">0</span></div>
          </td>
        </tr>
        </table>
    </div>
  ...
  <div class="col1"><div class="col1-inner">
    ...
    <div style="background-color:rgba(50,50,50,1); padding:10px;">
      <div id="placeholder_bound" style="width:100%; height:500px;">
        <div id="placeholder" style="height:500px"></div>
      </div>
    </div>
          
    <div id="power-graph-footer" style="background-color:#333; color:#999; display:none">
    ...
    <div id="advanced-block" style="background-color:#333; padding:10px; display:none">
      <div style="color:#999">
        <table class="table">
          <tr>
          <th></th>
  ...
  <div class="col2">
    <div class="col2-inner">
      ...
      <div style="background-color:rgba(50,50,50,1); padding:10px; color:#333;">
          <span id="totals">
...
radioButtons.click(function() {
    ...
    if(viewmode == "bargraph"){
      //$(radioButtons[radioButtons.length]).prop("disabled",true)
      $(".viewhistory").trigger( "click" )
    }
  else{
     //$(radioButtons[radioButtons.length]).prop("disabled",false)
      powergraph_load()
      powergraph_draw()
   }
...
config.app = {
    ...
    "use_kwh":{"type":"feed", "autoname":"use_kwh", "engine":5, "description":"Time Of Use accumulated kWh"},
     "Solar":{"type":"feed", "autoname":"Solar", "engine":5, "description":"Solar (W)"},
    "Solar_kwh":{"type":"feed", "autoname":"Solar_kwh", "engine":5, "description":"Solar accumulated kWh"},
    "currency":{"type":"value", "default":"$", "name": "Currency", "description":"Currency symbol (£,$..)"},
    "annual_sub_cost":{"type":"value", "default":"120", "name": "Subscrition cost (£,$..)", "description":"annual subscrition cost"},
...
{    
    ...
    feeds["Solar"] = config.feedsbyid[config.app["Solar"].value];
    feeds["Solar_kwh"] = config.feedsbyid[config.app["Solar_kwh"].value];

    tiers = config.app["tier_cost"].value.split(",");
    annual_sub_cost = config.app["annual_sub_cost"].value;
...
function updater()
{
    feed.listbyidasync(function(result){
        ...
        $("#solar_now").html(Math.round(feeds["Solar"].value)+"W");
    });
}
            ...
            if (viewmode == "bargraph") {
                ...
                for (var a = 0; a < tier_names.length; a++) {
                  if (viewcostenergy=="energy") {
                    tier_vals[a] = bargraph_series[a+1].data[z][1];
                    total += tier_vals[a];
                  }else{
                      tier_vals[a] = bargraph_series[a+1].data[z][1];
                      total += tier_vals[a];
                ...
                if (viewcostenergy=="energy") {
                total += bargraph_series[0].data[z][1];
}
                if (viewcostenergy=="energy") {
                    ...
                    for (var a = tier_names.length - 1; a >= 0; a--) {
                        ...
                        text += "<br>" + "Solar" + ": " + (bargraph_series[0].data[z][1]).toFixed(1) + " kWh";
                } else {
                    text = date + "<br>Total: "+ config.app["currency"].value + (total +(annual_sub_cost/360)).toFixed(2);
                    for (var a = tier_names.length - 1; a >= 0; a--) {
                        if (tier_vals[a] == 0) continue;
...
function powergraph_load() 
{
    ...
    data["solar"] = feed.getdata(feeds["Solar"].id,start,end,interval,1,1);
    ...
    for (var a = 0; a < data["use"].length; a++) {
        ...
        var d = new Date(time);
        var hr = d.getHours() + Math.floor(d.getMinutes()/60*2)/2;
  ...
  if (isChecked[0] === 1){
    ...
    var sub=[];      
     for (var a = 0; a < data["solar"].length; a++) {
      sub.push( [data["use"][a][0] ,data["use"][a][1] - data["solar"][a][1]]);
     }
...
if (isChecked[1] === 1){
    ...
    powergraph_series.push({
          ...
          yaxis:2,
          color:"#F0F8FF",
          lines:{show:true, fill:0, lineWidth:2}
    });
  }
    var feedstats = {};
    feedstats["use"] = stats(data["use"]);
    feedstats["solar"] = stats(data["solar"]);
...
{   
    ...
    var hourly = [0,0.5, 1,1.5, 2, 2.5, 3, 3.5, 4, 4.5, 5, 5.5, 6,6.5, 7,7.5, 8,8.5, 9,9.5, 10,10.5, 11,11.5, 12,12.5, 13,13.5, 14,14.5, 15, 15.5, 16,16.5, 17,17.5, 18,18.5, 19,19.5, 20,20.5, 21,21.5, 22,22.5, 23];
  
    var elec_result = feed.getdataDMY_time_of_use(feeds["use_kwh"].id,start,end,"daily",JSON.stringify(hourly));
    var Solar_result = feed.getdataDMY_time_of_use(feeds["Solar_kwh"].id,start,end,"daily",JSON.stringify(hourly));
    ...
    for (var z in elec_result) {
        ...
        if (include){ elec_data.push(elec_result[z]);
          Solar_data.push(Solar_result[z]);
        }
    ...
    if (elec_data.length>0) {
       ...
       Solar_total_kwh=0;
        ...
        for (var z = 0; z < elec_data.length; z++)
        {
            ...
            for (var y = 0; y < 23; y ++) {          
                ...
                if (we_ph(d)) { // weekend
                    ...
                    Solar_kwh += (Solar_data[z][1][y+1] - Solar_data[z][1][y]);
                } else {
        ...
        // last period ends with the next days first value
            if (we_ph(d)) { // weekend             
                if ((z+1)<elec_data.length){
                 tier_kwh[weekend_tiers[23]] += elec_data[z+1][1][0] - elec_data[z][1][23];
                 Solar_kwh += Solar_data[z+1][1][0] - Solar_data[z][1][23];
               }
            } else {
                if ((z+1)<elec_data.length){
                  tier_kwh[weekday_tiers[23]] += elec_data[z+1][1][0] - elec_data[z][1][23];
                  Solar_kwh  += Solar_data[z+1][1][0] - Solar_data[z][1][23];
                }
            }
            ...
            if (viewcostenergy=="energy") {  
                data_Solar.push([time,Solar_kwh]);             
                Solar_total_kwh += Solar_kwh;
              } else {
                data_Solar.push([time,0]);
                Dayly_sub_cost.push([time, annual_sub_cost/360]);
               }     
            
            n++;
        }
        period_average = total_kwh / n;
        sub_cost = annual_sub_cost / 360 * n
    }
  ...
  if (isChecked[1] === 1){
     if (viewcostenergy=="cost") {
       bargraph_series.push({
              stack: true,
              data: Dayly_sub_cost, color: "#B0C4DE",
              bars: { show: true, align: "left", barWidth: 0.8*3600*24*1000, fill: 1.0, lineWidth:0}
          });
    ...
    for (var a = 0; a < tier_names.length; a++) {
      ...
      if (viewcostenergy=="energy") {
        var totals_str = '<div class="electric-title">TOTAL</div><div class="power-value">' +
            (total_kwh + Solar_total_kwh).toFixed(1) + ' kWh</div><br>';
        var averages_str = '<div class="electric-title">TOTAL</div><div class="power-value">' +
           ((total_kwh + Solar_total_kwh)/n).toFixed(1) + ' kWh/d</div><br>';
        for (var a = 0; a < tier_names.length; a++) {
         ...
         totals_str += '<div class="electric-title">' + 'SOLAR' +
               '</div><div class="power-value">' + Solar_total_kwh.toFixed(1) + ' kWh</div><br>';
         averages_str += '<div class="electric-title">' + 'SOLAR' +
               '</div><div class="power-value">' + (Solar_total_kwh/n).toFixed(1) + ' kWh/d</div><br>';
        ...
        var totals_str = '<div class="electric-title">COMBINED</div><div class="power-value">' +
            config.app["currency"].value + (total_kwh + annual_sub_cost).toFixed(2) + '</div><br>';
        var averages_str = '<div class="electric-title">COMBINED</div><div class="power-value">' +
           config.app["currency"].value + ((total_kwh + sub_cost)/n).toFixed(2) + '/day</div><br>';
        for (var a = 0; a < tier_names.length; a++) {
            totals_str += '<div class="electric-title">' + tier_names[a].toUpperCase() +
    ...
    var kwh_today = 0;
    var solar_kwh_today = data_Solar[data_Solar.length-1];
    ...
    if (viewcostenergy=="energy") {
        ...
        $("#solar_kwh_today").html(solar_kwh_today[1].toFixed(1)+" kWh");
    } else {
        $("#kwh_today").html(config.app["currency"].value + kwh_today.toFixed(2));
         $("#solar_kwh_today").html(config.app["currency"].value + 0);
    }
...
function resize() {
    ...
    if (width<=500) {
        ...
        $(".solar-title").css("font-size","16px");
        $(".power-value").css("font-size","38px");
        $(".soalarpower-value").css("font-size","38px");
    } else if (width<=724) {
        $(".electric-title").css("font-size","18px");
         $(".solar-title").css("font-size","18px");
        $(".power-value").css("font-size","42px");
        $(".soalarpower-value").css("font-size","42px");
    } else {
        $(".electric-title").css("font-size","22px");
        $(".solar-title").css("font-size","22px");
        $(".power-value").css("font-size","42px");
         $(".soalarpower-value").css("font-size","42px");
    }
}
